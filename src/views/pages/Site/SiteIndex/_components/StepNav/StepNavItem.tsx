...
function StepNavItem({ data, item, activeMenu, index }:StepItemProps) {
    
    const stepName = item.name
    
    const isActive = item === activeMenu 
    const isComplete = index < data.findIndex((item:any) => item === activeMenu) 
    const isUpcoming = index > data.findIndex((item:any) => item === activeMenu)
